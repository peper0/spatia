...
int main() {
    ...
    const AziElev target_angles = EulerZY<GndNed>{Angle::from_degrees(30.0), Angle::from_degrees(10.0)};
    const Dir<GndNed> target_direction = to_dir(target_angles);
    const AziElev restored_target = to_euler_zy(target_direction);

    const PanTilt body_angles = EulerZY<BodyNed>{Angle::from_degrees(15.0), Angle::from_degrees(-5.0)};
    const Dir<BodyNed> body_direction = to_dir(body_angles);

    const YawPitchRoll attitude =
        EulerZYX<BodyNed, GndNed>{Angle::from_degrees(20.0), Angle::from_degrees(3.0), Angle::from_degrees(-1.0)};
    ...
    const PanTiltRoll camera_mount =
        EulerZYX<Camera, BodyNed>{Angle::from_degrees(2.0), Angle::from_degrees(-10.0), Angle{}};
