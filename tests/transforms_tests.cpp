...
class BiTransform {
   public:
    ...
    SRC operator()(const DST& p, Tag<SRC> = {}) const {
...
TEST(ComposeTest, HandlesBidirectionalTransforms) {
    ...
    auto t_multi = compose_transforms(t_a_from_b, t_b_c, t_b_from_a);
...
TEST(ComposeTest, SupportsOverloadedFunctionObject) {
    ...
    auto t_multi = compose_transforms(overloaded, c_from_d);
...
TEST(ComposeTest, SupportsAllSelectedOverloadsOfOrdinaryFunction) {
    ...
    auto t_multi = compose_transforms(
